...
class ArtifactModelManager(models.Manager):
    ...
    def with_optimized_details(self):
        return self.with_photos_and_categories(
        ).with_related(
            'finder', 'geography','category'
        ).prefetch_related(
            'photos__photo_type', 'materials'
        )

    def with_details(self):
        return self.with_photos_and_categories().with_related('finder', 'geography', 'category')

    def with_count_by_category(self):
        return self.values('category__name').annotate(artifact_count=Count('id'))
...
class Artifact(Physical):
    ...
    def create_initial_provenance_event(self):
        # Check if an event of the same type already exists
        event_type = 'Found'
        existing_event = ProvenanceEvent.objects.filter(
            artifact=self,
            event_type=event_type
        ).exists()

        # If an event doesn't exist, create the initial ProvenanceEvent
    ...
    def create_acquisition_provenance_event(self, collection):
        # Create an acquisition ProvenanceEvent when an artifact is acquired
        event_type = self.EventType.ACQUIRED
        c = ContentType.objects.get_for_model(self)
        ProvenanceEvent.objects.create(
            artifact=self,
            event_type=event_type,
...
class Collection(models.Model, EventActorMixin):
    name = models.CharField(max_length=255, db_index=True)
    ...
    def get_collections_owned_by_person(self, person):
        return Collection.objects.get_owned_by_person(person)

    def get_collection_trades(self):
        return Trade.objects.filter(Q(from_collection=self) | Q(to_collection=self)).order_by('date')

    def create_initial_provenance_event(self):
        # Create an initial ProvenanceEvent when a collection is created
        event_type = self.EventType.CREATED
        c = ContentType.objects.get_for_model(self)
        ProvenanceEvent.objects.create(
            collection=self,
            event_type=event_type,
...
class ProvenanceEventManager(models.Manager):
    ...
    def get_event_artifact(self):
        return self.artifact

    def get_events_for_artifact(self):
        return self.filter(artifact=self.artifact).order_by('date')


    def by_type(self, t):
        return self.filter(event_type=t)
...
class ProvenanceEvent(models.Model):
    ...
    def update_provenance_event(self, description):
        # Update a provenance event and trigger the UPDATED event type
        self.description = description
        self.save()

        # Trigger the UPDATED event
        event_type = self.EventType.UPDATED
        c = ContentType.objects.get_for_model(self)
        ProvenanceEvent.objects.create(
